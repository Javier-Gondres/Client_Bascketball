...
import { Equipo } from "@/interfaces/entities";
...
const EquiposDashboard = () => {
   const [entidades, setEntidades] = useState<Equipo[]>([]);
   ...
   const fetchData = useCallback(async () => {
      try {
         const response = await fetch(`${ports.api}/equipo`);
   ...
   const createEntity = async (formData: Partial<Equipo>) => {
      try {
         const response = await fetch(`${ports.api}/equipo`, {
            method: "POST",
   ...
   const updateEntity = async (codigo: string, formData: Partial<Equipo>) => {
      try {
         console.log({ codigo });
         ...
         const data: Equipo = await response.json();
         setEntidades((prev) =>
            prev.map((e) => (e.CodEquipo === data.CodEquipo ? data : e))
         );
         showMessage({
            message: "Equipo actualizada",
            backgroundColor: colors.blue.blue400,
            textStyle: { fontSize: fontSizes.medium },
         });
         console.log("Entidad actualizado:", data.CodEquipo);
      } catch (error) {
   ...
   const deleteEntity = async () => {
      try {
         ...
         const codEntidadAEliminar = entidadesAEliminar.map((j) => j.CodEquipo);

         const results = await Promise.allSettled(
            codEntidadAEliminar.map((codJugador) =>
               fetch(`${ports.api}/equipo/${codJugador}`, { method: "DELETE" })
            )
         );
         ...
         if (eliminados.length > 0) {
            setEntidades((prev) =>
               prev.filter((jugador) => !eliminados.includes(jugador.CodEquipo))
            );
   ...
   return (
      <SafeAreaView style={[styles.container, { paddingTop: insets.top + 20 }]}>
         <View style={{ flex: 1, gap: spacings.s2 }}>
            ...
            <FlatList
               data={filteredData}
               keyExtractor={(item) => item.CodEquipo}
               numColumns={2}
                     ...
                     if (crudMode === "delete") {
                        setSelectedToRemove((prev) => {
                           ...
                           if (isSelectedToRemove) {
                              newSelection.delete(item.CodEquipo);
                           } else {
                              newSelection.set(item.CodEquipo, item);
                           }
                           return newSelection;
                        });
                     } else {
                  ...
                  return (
                     ...
                     >
                        <LottieView
                           ...
                           source={require("../../assets/lotties/pelotaSimple.json")}
                        />
         ...
         >
            <EquipoFormulario
               ...
               onAccept={async (form) => {
                  if (selected) {
                     await updateEntity(selected.CodEquipo, {
                        CodCiudad: form.ciudad?.CodCiudad ?? null,
                        Nombre: form.Nombre,
                     });
                  } else {
                     await createEntity({
