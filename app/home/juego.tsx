...
import { Juego } from "@/interfaces/entities";
...
const JuegosDashboard = () => {
   const [entidades, setEntidades] = useState<Juego[]>([]);
   const [searchQuery, setSearchQuery] = useState("");
   const [selected, setSelected] = useState<Juego | null>(null);
   const [selectedToRemove, setSelectedToRemove] = useState<Map<string, Juego>>(
      new Map()
   );
   ...
   const fetchData = useCallback(async () => {
      try {
         const response = await fetch(`${ports.api}/juego`);
   ...
   const createEntity = async (formData: Partial<Juego>) => {
      try {
         const response = await fetch(`${ports.api}/juego`, {
            method: "POST",
   ...
   const updateEntity = async (codigo: string, formData: Partial<Juego>) => {
      try {
         ...
         const data: Juego = await response.json();
         setEntidades((prev) =>
            prev.map((e) => (e.CodJuego === data.CodJuego ? data : e))
         );
         showMessage({
         ...
         console.log("Entidad actualizado:", data.CodJuego);
      } catch (error) {
   ...
   const deleteEntity = async () => {
      try {
         ...
         const codEntidadAEliminar = entidadesAEliminar.map((j) => j.CodJuego);

         const results = await Promise.allSettled(
            codEntidadAEliminar.map((codJugador) =>
               fetch(`${ports.api}/juego/${codJugador}`, { method: "DELETE" })
            )
         );
         ...
         if (eliminados.length > 0) {
            setEntidades((prev) =>
               prev.filter((jugador) => !eliminados.includes(jugador.CodJuego))
            );
   ...
   return (
      <SafeAreaView style={[styles.container, { paddingTop: insets.top + 20 }]}>
         <View style={{ flex: 1, gap: spacings.s2 }}>
            ...
            <FlatList
               data={filteredData}
               keyExtractor={(item) => item.CodJuego}
               numColumns={2}
                     ...
                     if (crudMode === "delete") {
                        setSelectedToRemove((prev) => {
                           ...
                           if (isSelectedToRemove) {
                              newSelection.delete(item.CodJuego);
                           } else {
                              newSelection.set(item.CodJuego, item);
                           }
                           return newSelection;
                        });
                     } else {
                  ...
                  return (
                     ...
                     >
                        <LottieView
                           ...
                           source={require("../../assets/lotties/manoPrieta.json")}
                        />
                        <View style={styles.playerInfoContainer}>
                           ...
                           {item.Equipo1Entity && (
                              ...
                              >
                                 Equipo Local: {item.Equipo1Entity.Nombre}
                              </Text>
                           )}
                           {item.Equipo2Entity && (
                              <Text
                              ...
                              >
                                 Equipo Local: {item.Equipo2Entity.Nombre}
                              </Text>
                           )}
                        </View>
                     </TouchableOpacity>
         ...
         >
            <JuegoFormulario
               ...
               onAccept={async (form) => {
                  if (selected) {
                     await updateEntity(selected.CodJuego, {
                        Descripcion: form.Descripcion,
                        Equipo1: form.Equipo1Entity.CodEquipo,
                        Equipo2: form.Equipo2Entity.CodEquipo,
                        Fecha: moment(form.Fecha)
                           .set({
                              hour: 12,
                              minute: 0,
                              second: 0,
